...
function getSpacing(node, type, suffix, locations) {
  for (var i = 0; i < locations.length; ++i) {
    ...
    let key = type + capitalizeFirst(location) + suffix;
...
function layoutNode(node, parentMaxWidth, /*css_direction_t*/parentDirection) {
  let /*css_direction_t*/ direction = resolveDirection(node, parentDirection);
  let /*css_flex_direction_t*/ mainAxis = resolveAxis(getFlexDirection(node), direction);
  let /*css_flex_direction_t*/ crossAxis = getCrossFlexDirection(mainAxis, direction);
  let /*css_flex_direction_t*/ resolvedRowAxis = resolveAxis(CSS_FLEX_DIRECTION_ROW, direction);
  ...
  if (isMeasureDefined(node)) {
    ...
    let /*bool*/ isRowUndefined = !isDimDefined(node, resolvedRowAxis) &&
  ...
  while (endLine < node.children.length) {
    ...
    for (i = startLine; i < endLine; ++i) {
      ...
      if (getPositionType(child) === CSS_POSITION_ABSOLUTE &&
        ...
        let /*float*/ leadingCrossDim = getLeadingPaddingAndBorder(node, crossAxis);
  ...
  if (linesCount > 1 && !isUndefined(node.layout[dim[crossAxis]])) {
    let /*float*/ nodeCrossAxisInnerSize = node.layout[dim[crossAxis]] -
      getPaddingAndBorderAxis(node, crossAxis);
    let /*float*/ remainingAlignContentDim = nodeCrossAxisInnerSize - linesCrossDim;

    let /*float*/ crossDimLead = 0;
    let /*float*/ currentLead = getLeadingPaddingAndBorder(node, crossAxis);
...
function fillNodes(originalNode) {
  let clonedStyle = {};
  if (originalNode.props && originalNode.props.style) {
    const keys = Object.keys(originalNode.props.style);
    for (let i = 0, l = keys.length; i < l; i++) {
      const key = keys[i];
      clonedStyle[key] = originalNode.props.style[key]
    }
  }
  ...
  if (originalNode && originalNode.props && originalNode.props.children) {
    originalNode.props.children.forEach(function(child, i) {
      xNode.children[i] = fillNodes(child);
    });
  }
