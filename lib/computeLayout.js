...
function getJustifyContent(node) {
  if (node.props && node.props.style && 'justifyContent' in node.props.style) {
    return node.props.style.justifyContent;
  }
  return 'flex-start';
}

function getAlignContent(node) {
  if (node.props && node.props.style && 'alignContent' in node.props.style) {
    return node.props.style.alignContent;
  }
  return 'flex-start';
}

function getAlignItem(node, child) {
  if (node.props && node.props.style && 'alignSelf' in child.props.style) {
    return child.props.style.alignSelf;
  }
  if (node.props && node.props.style && 'alignItems' in node.props.style) {
...
function resolveDirection(node, parentDirection) {
  var direction;
  if (node.props && node.props.style && 'direction' in node.props.style) {
    direction = node.props.style.direction;
  }
  else {
...
function getFlexDirection(node) {
  if (node.props && node.props.style && 'flexDirection' in node.props.style) {
    return node.props.style.flexDirection;
  }
...
function getPositionType(node) {
  if (node.props && node.props.style && 'position' in node.props.style) {
    return node.props.style.position;
  }
  return 'relative';
}

function getFlex(node) {
  return node.props.style.flex;
}
...
function isFlexWrap(node) {
  return node.props && node.props.style && node.props.style.flexWrap === 'wrap';
}
...
function isDimDefined(node, axis) {
  return node.props && node.props.style && !isUndefined(node.props.style[dim[axis]]) && node.props.style[dim[axis]] >= 0;
}

function isPosDefined(node, pos) {
  return node.props && node.props.style && !isUndefined(node.props.style[pos]);
}

function isMeasureDefined(node) {
  return node.props && node.props.style && 'measure' in node.props.style;
}

function getPosition(node, pos) {
  if (node.props && node.props.style && pos in node.props.style) {
    return node.props.style[pos];
  }
  return 0;
}
...
function boundAxis(node, axis, value) {
  ...
  var min = {
    'row': node.props.style ? node.props.style.minWidth : 0,
    'row-reverse': node.props.style ? node.props.style.minWidth  : 0,
    'column': node.props.style ? node.props.style.minHeight : 0,
    'column-reverse': node.props.style ? node.props.style.minHeight : 0
  }[axis];

  var max = {
    'row': node.props.style ? node.props.style.maxWidth : 0,
    'row-reverse': node.props.style ? node.props.style.maxWidth : 0,
    'column': node.props.style ? node.props.style.maxHeight : 0,
    'column-reverse': node.props.style ? node.props.style.maxHeight : 0
  }[axis];
...
function setDimensionFromStyle(node, axis) {
  ...
  node.layout[dim[axis]] = fmaxf(
    boundAxis(node, axis, node.props.style[dim[axis]]),
...
function getRelativePosition(node, axis) {
  if (node.props && node.props.style && leading[axis] in node.props.style) {
...
function layoutNode(node, parentMaxWidth, /*css_direction_t*/parentDirection) {
  ...
  var children = node.props && node.props.children ? node.props.children : [];
  ...
  // The position is set by the parent, but we need to compvare it with a
  ...
  if (isMeasureDefined(node)) {
    ...
    // var's not measure the text if we already know both dimensions
    if (isRowUndefined || isColumnUndefined) {
      var /*css_dim_t*/ measureDim = node.props.style.measure(
        /*(c)!node->context,*/
  ...
  while (endLine < children.length) {
    ...
    var /*float*/ maxWidth;
    for (i = startLine; i < children.length; ++i) {
      child = children[i];
  ...
  // section 9.4
  //
  if (linesCount > 1 && !isUndefined(node.layout[dim[crossAxis]])) {
    ...
    for (i = 0; i < linesCount; ++i) {
      ...
      for (ii = startIndex; ii < children.length; ++ii) {
        child = children[ii];
        if (getPositionType(child) !== CSS_POSITION_RELATIVE) {
          continue;
        }
