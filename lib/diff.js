...
function handleProperty(newNode, oldNode, newNodeKeys, i, isDifferent) {
  ...
  if (key !== 'layout') {
    ...
    if (key === 'children' && isArray(newValue)) {
      ...
      if (isDifferent) {
        ...
        for (var k = 0, j = newValue.length; k < j; k++) {
          if (hasOnlyTextChildren && typeof newValue[k] !== 'string') {
  ...
  return {
    isDifferent: isDifferent,
    hasOnlyTextChildren: hasOnlyTextChildren
  };
}
...
function diff(newNode, oldNode, parent, direction) {
  ...
  if (isArray(newNode)) {
    var diffedArray = diffArray(newNode, oldNode, parent, direction);
    isDifferent = isDifferent || diffedArray;
  }
  ...
  if (newNode) {
    addLayoutInformation(newNode, isDifferent, direction);
  }
...
function addLayoutInformation(node, isDifferent, direction) {
  if (node.props) {
    if (isDifferent && node.props && node.props.style) {
      node.__bluebox.direction = 'direction' in node.props.style ? node.props.style.direction : direction;
      node.__bluebox.flexDirection = 'flexDirection' in node.props.style ? node.props.style.flexDirection : 'column';
      node.__bluebox.crossFlexDirection = node.__bluebox.flexDirection === 'column' ? 'row' : 'column';
      //node.__bluebox.paddingAndBorderAxis =
    }
    else {
      node.__bluebox.direction = direction;
      node.__bluebox.flexDirection = 'column';
    }
  }
}
