...
describe('diff', function() {
  ...
  it('should diff items with the same key in an array with keyed components', function() {
    var oldObj = {children: [{props: {a:{o: 1}, key:1}}, {props:{b:{o: 3}, key:2}}]};
    var newObj = {children: [{props: {z:{o: 2}, key:0}}, {props:{a:{o: 1}, key:1}}, {props:{b:{o:3}, key:2}}]};
    var result = diff(newObj, oldObj);
    expect(result).not.toBe(oldObj);
    expect(result.children).not.toBe(oldObj.children);
    expect(result.children[1]).toBe(oldObj.children[0]);
    expect(result.children[2]).toBe(oldObj.children[1]);
  });
